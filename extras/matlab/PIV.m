%   PIV: generate the PIV data from the images loaded with the input parameters.
...
function [data]=PIV(images,input_parameters)
    ...
    for current_pass=1:length(parameters.IntWin)
        ...
        q=quiver(data.x(ix_vectors,iy_vectors),data.y(ix_vectors,iy_vectors),data.u(ix_vectors,iy_vectors),data.v(ix_vectors,iy_vectors),5);shading interp;view(0,90);
        set(q,'color','y')
