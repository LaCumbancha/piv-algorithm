...
def calculate_piv(frontend_data):
    ...
    #   Using Python's multiprocessing increase the response time substantially.
    for point_id, point_data in frontend_data.points.items():
        ...
        if settings.roi_size is not None:
            ...
            shift_x = max(point_data.pos_x - roi_shift, 0)
            shift_y = max(point_data.pos_y - roi_shift, 0)
        
        piv_data = core.piv(double_framed_images, settings.window_size)
        piv_data.x = (piv_data.x + shift_x) * settings.scale
        piv_data.y = (piv_data.y + shift_y) * settings.scale
